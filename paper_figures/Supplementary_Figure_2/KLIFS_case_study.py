...
def run_example(ensemble_dict, rseed=123):
    ...
    for case_study in ensemble_dict:
        ...
        for ens_dir in [on_target_dir, off_target_dir]:
            ...
            for p in in_prots:
                if not Path(str(p).replace('.mol2', '_aligned.mol2')).exists():
	                ...
	                if Path(str(p).replace('protein.mol2', 'ligand.mol2')).exists():
	                ...
	                complexa = Protein.from_file(str(p).replace('protein.mol2', 'complex.mol2'))
	                complexa.transform(transformation)
            ...
            subset_df = get_subset(ensemble_df, data_source='KLIFS', mw_threshold=1000, unique_ligands=True)
        ...
        for ens_dir in [on_target_dir, off_target_dir]:
            ...
            # shrunk_hot_paths = [Path(hp.parent, 'binding_site_maps', 'out.zip') for hp in hot_paths]
            # shrunk_hotspots = []
            #
            # for shp in shrunk_hot_paths:
            #     with HotspotReader(str(shp)) as hs_reader:
            #         sh_res = hs_reader.read()
            #         shrunk_hotspots.append(sh_res)
...
#                      }
    ...
    e_dict = {'case_study_1':
                  {'on_target_dir': Path('p38a_all'),
                   'off_target_dir': Path('Erk2_all'),
                   'ref_structure_path': Path('p38a_all/1w7h_chainA/protein.mol2')
                   }
              }
