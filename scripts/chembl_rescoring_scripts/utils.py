...
def get_subset(path_to_ensemble_data, data_source, mw_threshold=300.0, unique_ligands=True):
    ...
    if data_source == 'SIENA':
        ...
        # the size of the ligands is already taken into account. NO!
        df = df[df["Ligand MW"] <= mw_threshold]
        df = df[df['alt'] != 'B']
        print(df.columns)
...
#         cur_prot = protoss_res.protein
        ...
        if transformation:
            ...
            for bl in bs_ligands:
                bl.transform(transformation)
...
#         grid_min_coords.append(hs_res.super_grids['apolar'].bounding_box[0])
#         grid_max_coords.append(hs_res.super_grids['apolar'].bounding_box[1])
        
#     grid_min_coords_arr = np.array(grid_min_coords)
#     grid_max_coords_arr = np.array(grid_max_coords)
    
#     if (max_coords > np.max(grid_max_coords_arr, axis=0)).any():
#         max_coords = np.min(grid_max_coords_arr, axis=0)
        
#     if (min_coords < np.min(grid_min_coords_arr, axis=0)).any():
#         min_coords = np.max(grid_min_coords_arr, axis=0)
    ...
    for hs_path in hotspot_paths:
        print(hs_path)
        #if not Path(hs_path.parent, 'binding_site_maps', 'out.zip').exists():
        with HotspotReader(str(hs_path)) as hs_reader:
            hs_res = hs_reader.read()
...
#     else:
#         with HotspotReader(str(Path(hs_path.parent, 'binding_site_maps', 'out.zip'))) as hrd:
#             shrunk = hrd.read()
#         shrunk_hs_results.append(shrunk)
#         shrunk_hs_result_paths.append(Path(hs_path.parent, 'binding_site_maps'))
...
if __name__ == '__main__':
    ...
    thiolate_path = Path('/home/jin76872/Desktop/Mih/Data/covid_screen/dimer_hotspots/thiolate')
    save_path = Path(thiolate_path, 'scored_receptors')
    hotspot_paths = list(Path(thiolate_path).glob('*/fullsize_hotspots_3000/out.zip'))

    input_list = [(i, hs_path, save_path) for i, hs_path in enumerate(hotspot_paths)]

    with Pool(processes=15) as pool:
        pool.map(score_hotspot_protein, input_list)
