...
def query_Siena(pdb_code, ligand_name, out_dir="", reduction_procedure="", bb_reduction=""):
    """
    Use Pete's script to query the SIENA restful API; change the search parameters a little from what he had
    :param pdb_code:
    :param ligand_name:
    :param chain:
    :return:
    ...
    ssettings.data["reduction_procedure"] = reduction_procedure
    ssettings.data["siena"]["ligand"] = ligand_name
    ssettings.data["siena"]["bb_clustering"] = bb_reduction
...
class SienaQuery():
    def __init__(self, target_name, pdb_code, ligand_name, siena_dir='', reduction_procedure='', bb_reduction=''):
        self.ensemble_name = target_name
        self.query_pdb_code = pdb_code
        self.query_ligand_name = ligand_name
        self.siena_dir = siena_dir
        self.reduction_procedure = reduction_procedure
        self.bb_reduction = bb_reduction
        ...
        if not self.siena_resultstat.exists():
            query_Siena(self.query_pdb_code, self.query_ligand_name, self.siena_dir, self.reduction_procedure, self.bb_reduction)
...
if __name__ == "__main__":
    brd1_dir = Path('/home/jin76872/Desktop/Mih/Data/JCIM_selectivity_paper/case_studies/bromodomains/BRD1')
    if not brd1_dir.exists(): brd1_dir.mkdir()

    brd1_query = SienaQuery(target_name='BRD1', pdb_code='5AMF', ligand_name='TWL_A_1143', siena_dir=brd1_dir)
    brd1_query.get_ensemble_protein_df()
    brd1_sub_ens =get_subset(Path(brd1_dir, 'ensemble_data.csv'), mw_threshold=300.0, unique_ligands=True)
    brd1_sub_ens.to_csv(str(Path(brd1_dir, 'subset_ensemble_data.csv')))

    brpf1_dir = Path('/home/jin76872/Desktop/Mih/Data/JCIM_selectivity_paper/case_studies/bromodomains/BRPF1')
    if not brpf1_dir.exists(): brpf1_dir.mkdir()
    brpf1_query = SienaQuery(target_name='BRPF1', pdb_code='5OWE', ligand_name='B0E_A_801', siena_dir=brpf1_dir)
    brpf1_query.get_ensemble_protein_df()
    brpf1_sub_ens =get_subset(Path(brpf1_dir, 'ensemble_data.csv'), mw_threshold=300.0, unique_ligands=True)
    brpf1_sub_ens.to_csv(str(Path(brpf1_dir, 'subset_ensemble_data.csv')))