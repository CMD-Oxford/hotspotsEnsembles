...
ensemble_dict = {'on_target':{'target_name': 'BRD1',
                              'pdb_code': '5AMF',
                              'ligand_name':'TWL_A_1143'},
                 'off_target': {'target_name': 'BRPF1',
                                'pdb_code': '5OWE',
                                'ligand_name': 'B0E_A_801'}}

# Get the ensembles from SIENA. Remove redundant ligands (keep highest res) and anything > 300 Da
for ens in ensemble_dict.keys():
    ens_dir = Path(f"../case_studies/bromodomains_protoss_csd2021/{ensemble_dict[ens]['target_name']}")
    ...
    if not Path(ens_dir, 'subset_ensemble_data.csv').exists():
        ...
        ens_query = SienaQuery(target_name=ensemble_dict[ens]['target_name'],
                               ...
                               ligand_name=ensemble_dict[ens]['ligand_name'], siena_dir=ens_dir)

        ens_query.get_ensemble_protein_df()
    ens_sub_ens =get_subset(Path(ens_dir, 'ensemble_data.csv'), data_source='SIENA', mw_threshold=300.0)
...
target_prot_path = list(Path(ensemble_dict['on_target']['ensemble_df_path'].parent, 'pdb_files').glob(f'{ensemble_dict["on_target"]["pdb_code"]}_*.pdb'))[0]
target_ref_prot = Protein.from_file(str(target_prot_path))
...
tar_binding_site = Protein.BindingSiteFromMolecule(target_ref_prot,target_ref_prot.ligands[0], 6.0)
(rmsd, transform) = sup.superpose(target_ref_prot.chains[0], off_target_ref_prot.chains[0], tar_binding_site)
print(rmsd, transform)

on_tar_paths_chains = process_siena_pdbs(res_table_path=ensemble_dict['on_target']['ensemble_df_path'])
